...
if(isset($language) && preg_match('/^([a-z]{2})/', $language, $matches)) if($lang == 'xx') $lang = $matches[1];
if(!isset($txt['smf_repair_settings'][$lang])) $lang = 'en';
...
if (!empty($db_type) && isset($txt['db_' . $db_type][$lang]))
	$txt['database_settings'][$lang] = $txt['db_' . $db_type][$lang] . ' ' . $txt['database_settings'][$lang];
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>', isset($context['smfVersion']) ? sprintf($txt['smf_repair_settings'][$lang], $context['smfVersion']) : 
$txt['smf11_repair_settings'][$lang], '</title>
		...
		<style type="text/css">
			...
			td.textbox
			{
				...
				font-weight: bold;
				white-space: nowrap;
				padding-', empty($txt['lang_rtl'][$lang]) ? 'right' : 'left', ': 2ex;
			}
			.smalltext
			{
				font-size: 0.8em;
	...
	<body>
		<div id="header">
			...
			<div>', isset($context['smfVersion']) ? sprintf($txt['smf_repair_settings'][$lang], $context['smfVersion']) : 
$txt['smf11_repair_settings'][$lang], '</div>
		</div>
		<div id="content">';
...
{
	global $txt, $lang, $smcFunc, $db_connection, $db_type, $db_name, $db_prefix, $context, $sources_exist, $sources_found_path, $db_type_options;
...
*/
	...
	if (!empty($theme_settings))
	{
		...
		foreach ($theme_settings as $id => $theme)
		{
			...
			$txt['theme_' . $id . '_theme_url'][$lang] = $theme['name'] . ' URL';
			$txt['theme_' . $id . '_images_url'][$lang] = $theme['name'] . ' Images URL';
			$txt['theme_' . $id . '_theme_dir'][$lang] = $theme['name'] . ' Directory';
		}
	}
	...
	if ($db_connection == true)
	{
		...
		if (!empty($context['hooks_removed']))
		{
			echo '
				<div class="success_message" style="margin-bottom: 2ex;">
					', $txt['hooks_' . ($context['smfVersion'] == '2.1' ? 'disabled' : 'removed')][$lang], '
				</div>';
		}
	...
	elseif (empty($show_db_settings))
	{
		echo '
			<div class="error_message" style="margin-bottom: 2ex;">
				', $txt['database_settings_hidden'][$lang], '
			</div>';
	}
	...
	{
		echo '
					<h2>', $txt[$settings_section][$lang], '</h2>
					<h3>', $txt[$settings_section . '_info'][$lang], '</h3>
		...
		foreach ($section as $setting => $info)
		{
			...
			echo '
							<td width="20%" valign="top" class="textbox" style="padding-bottom: 1ex;">
								<label', $info[1] != 'int' ? ' for="' . $setting . '"' : '', '>', $txt[$setting][$lang], ': '.
									( isset($txt[$setting . '_desc'][$lang]) ? '<span class="smalltext">' . $txt[$setting . '_desc'][$lang] . '</span>' : '' ).'
			...
			if ($info[1] == 'int' || $info[1] == 'check')
			{
				for ($i = 0; $i <= $info[2]; $i++)
					echo '
								<label for="', $setting, $i, '"><input type="radio" name="', $info[0], 'settings[', $setting, ']" id="', $setting, $i, '" value="', $i, '"', isset($settings[$setting]) && $settings[$setting] == $i ? ' checked="checked"' : '', ' class="input_radio" /> ', $txt[$setting . $i][$lang], '</label><br />';
			}
			elseif ($info[1] == 'string')
			{
				...
				if (isset($txt[$setting . '_help'][$lang]))
					echo '<div style="font-size: smaller;">', $txt[$setting . '_help'][$lang], '</div>';

				if (isset($info[2]))
					echo '
								<div style="font-size: smaller;">', $txt['default_value'][$lang], ': &quot;<strong><a href="javascript:void(0);" id="', $setting, '_default" onclick="document.getElementById(\'', $setting, '\').value = ', $info[2] == '' ? '\'\';">' . $txt['recommend_blank'][$lang] : 'getInnerHTML(this);">' . $info[2], '</a></strong>&quot;.</div>',
			...
			elseif ($info[1] == 'array_string')
			{
				...
				foreach ($array_settings as $array_setting)
				{
					...
					{
						echo '
								<div style="font-size: smaller;">', $txt['default_value'][$lang], ': &quot;<strong><a href="javascript:void(0);" id="', $setting, $item, '_default" onclick="document.getElementById(\'', $setting, $item, '\').value = ', $suggested[0] == '' ? '\'\';">' . $txt['recommend_blank'][$lang] : 'getInnerHTML(this);">' . $suggested[0], '</a></strong>&quot;.</div>',
								$suggested[0] == '' ? '' : '
								<script type="text/javascript"><!-- // --><![CDATA[
						...
						for ($i = 1; $i < count($suggested); $i++)
							echo '
								<div style="font-size: smaller;">', $txt['other_possible_value'][$lang], ': &quot;<strong><a href="javascript:void(0);" id="', $setting, $item, '_default" onclick="document.getElementById(\'', $setting, $item, '\').value = ', $suggested[$i] == '' ? '\'\';">' . $txt['recommend_blank'][$lang] : 'getInnerHTML(this);">' . $suggested[$i], '</a></strong>&quot;.</div>';
					}
					else
						echo '
								<div style="font-size: smaller;">', $txt['no_default_value'][$lang], '</div>';
			...
			elseif ($info[1] == 'select' && !empty($info[3]))
			{
				...
				if (!is_null($info[2]))
					echo '
								<div style="font-size: smaller;">', $txt['default_value'][$lang], ': &quot;<strong><a href="javascript:void(0);" id="', $setting, '_default" onclick="$(\'#', $setting, '\').val(\'', $info[2], '\').change();">' . $info[3][$info[2]], '</a></strong>&quot;.</div>',
								$info[2] == '' ? '' : ($setting != 'language' && $setting != 'cookiename' ? '
								<script type="text/javascript"><!-- // --><![CDATA[
									resetSettings[settingsCounter++] = "' . $setting . '"; // ]]></script>' : '');
			}
	...
	if ($failure)
		echo '
				<input type="submit" name="submit" value="', $txt['save_settings'][$lang], '" disabled="disabled" class="button_submit" /><br />', $txt['not_writable'][$lang];
	else
	{
		echo '
				[<a href="javascript:restoreAll();">', $txt['restore_all_settings'][$lang], '</a>]
				<input type="submit" name="submit" value="', $txt['save_settings'][$lang], '" class="button_submit" />';

		if (!$context['is_legacy'])
			echo '
				<input type="submit" name="remove_hooks" value="' . $txt[($context['smfVersion'] == '2.1' ? 'disable' : 'remove') . '_hooks'][$lang] . '" class="button_submit" />';
	}

	//In either case, an easy remove button? This has a long name in it, so let's break a line
	echo 		'<br /><input type="submit" name="remove_file" value="' . $txt['remove_file'][$lang] . '" class="button_submit" />';
